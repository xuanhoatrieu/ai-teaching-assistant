...
start_backend() {
    ...
    cd "$BASE_DIR/backend"
    setsid node dist/src/main.js > "$LOGS_DIR/backend.log" 2>&1 &
...
start_frontend() {
    ...
    setsid ./node_modules/.bin/vite --host 0.0.0.0 > "$LOGS_DIR/frontend.log" 2>&1 &
...
start_pptx() {
    ...
    setsid "$BASE_DIR/backend/utils/pptx_generator/venv/bin/uvicorn" main:app --host 0.0.0.0 --port 3002 > "$LOGS_DIR/pptx.log" 2>&1 &
...
start_worker() {
    ...
    setsid "$BASE_DIR/vid-worker/venv/bin/python" worker.py > "$LOGS_DIR/vid-worker.log" 2>&1 &
